// server.js
  ...
  // define model =================
  var userSchema = new Schema({ name: String, groups: String});

    var User = mongoose.model('users', userSchema);

    o.map = function () {
        for(var i in this.groups){
            key = { group: this.name.firstname + " - " + this.groups[i].action };
            value = this.groups[i].members;
            emit(key, value);
          }
    };
    ...
    o.reduce = function (key, values) {
         /*user_list = { members: [] };
          for(var i in values) {
                ...
                user_list.members = values[i].members.concat(results.name.firstname);
